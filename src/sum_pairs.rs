...
fn main() {
  let l1 = [1, 4, 8, 7, 3, 15];
  let l2 = [1, -2, 3, 0, -6, 1];
  let amount = sum_pairs(&l2, -6);
  println!("{:?}", amount);
}

fn sum_pairs_slow(ints: &[i8], s: i8) -> Option<(i8, i8)> {
  let mut found: Option<(i8, i8)> = None;
  let mut max_idx = ints.len();

  for i in 0..max_idx {
    if i >= max_idx {
      break;
    }
    for j in (i + 1..max_idx).rev() {
      println!("{}, {}, {} = {}", i, j, ints[i] + ints[j], s);

      if ints[i] + ints[j] == s && j < max_idx {
        found = Some((ints[i], ints[j]));
        max_idx = j + 1;
      }
    }
  }
...
fn sum_pairs(ints: &[i8], s: i8) -> Option<(i8, i8)> {
  let max_idx = ints.len();
  let mut map: HashMap<i8, i8> = HashMap::new();

  for i in 0..max_idx {
    if map.contains_key(&(s - ints[i])) {
      return Some((s - ints[i], ints[i]));
    }

    map.insert(ints[i], ints[i]);
  }
